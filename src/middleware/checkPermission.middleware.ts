...
function haveAccess(userRole: string, subject: string, action: string) {
  if (Object.prototype.hasOwnProperty.call(Permissions, subject)) {
    if (Permissions[userRole].level < Permissions[subject].level) {
      return true;
    }
  }
  if (!Object.prototype.hasOwnProperty.call(Permissions[userRole], subject)) {
    return false;
  }
  if (Permissions[userRole][subject].includes(action) || Permissions[userRole][subject].includes('full-control')) {
    return true;
  }
